...
int main() {
    ...
    server_address.sin_addr.s_addr = inet_addr("192.185.1.1");  // Change to the server's IP address
    ...
    // Client generates a random DES key
	int des_key[16];
	string original_string;
	for (int i = 0; i < 16; i++) {
	    int random_ascii;
	    if (i % 2 == 0) {
	        // Generate a random ASCII for '0'-'9'
	        random_ascii = rand() % 10 + 48;
	    } else {
	        // Generate a random ASCII for 'A'-'F'
	        random_ascii = rand() % 6 + 65;
	    }
	
	    des_key[i] = random_ascii;
	    original_string += char(des_key[i]);
	}
    
    // Display the decrypted string
    cout << "Original key: " << original_string << endl;

    // Encrypt the DES key using the public key
    int encrypted_des_key[16];
    cout << "DES Key (encrypted): ";
    for (int i = 0; i < 16; i++) {
        encrypted_des_key[i] = encrypt(des_key[i]);
        cout << encrypted_des_key[i] << " ";
    }
    cout << endl;

    // Send the encrypted DES key to the server
    send(client_socket, &encrypted_des_key, sizeof(encrypted_des_key), 0);

    cout << "Encrypted DES key sent successfully!\n\n";
    ...
    for (int i = 0; i < str.length(); i++) {
        ...
        input_message = input_message + ascii_table[find];
    }

    // Proses DES Encrypt
    string encrypted_message = des_process(input_message, original_string, true);
