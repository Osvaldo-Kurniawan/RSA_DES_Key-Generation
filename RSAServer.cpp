...
string xored_msg(string ep_msg, string fe_key) {
    int xor_val, len = ep_msg.length();
    string xored_msg;

    for (int i = 0; i < len; i++) {
        xor_val = ((fe_key[i] - '0') ^ (ep_msg[i] - '0'));
        xored_msg = xored_msg + char(xor_val + '0');
    }
    return xored_msg;
}
...
string convertToString(int decrypted_byte) {
    stringstream ss;
    ss << decrypted_byte;
    return ss.str();
}
...
int main() {
    ...
    server_address.sin_addr.s_addr = INADDR_ANY;  // Allow connections from any network interface
    ...
    cout << "Connection established with the client!\n";

    // Server waits for incoming messages
    while (true) {
        int encrypted_des_key[16];

        // Receive the encrypted DES key from the client
        ssize_t bytes_received = recv(client_socket, &encrypted_des_key, sizeof(encrypted_des_key), 0);
		...
		// Display the original DES key (received encrypted key)
        cout << "DES Key (encrypted): ";
        for (int i = 0; i < 16; i++) {
            cout << encrypted_des_key[i] << " ";
        }
        
        // Decrypt the DES key using the private key and convert to string
		string decrypted_string;
		for (int i = 0; i < 16; i++) {
		    int decrypted_byte = decrypt(encrypted_des_key[i]);
		    char ascii_char = static_cast<char>(decrypted_byte);
		    decrypted_string += ascii_char;
		}
		cout << endl;

        // Display the decrypted string
        cout << "Decrypted String: " << decrypted_string << endl << endl;
	    ...
	    string decrypted_message = des_process(received_message, decrypted_string, false);
	
	    cout << "Received Encrypted Message: " << received_message << endl;
	
		// Additional steps to convert decrypted message to plain text
	    int len=2;
		string substring="",plain_txt="";		
		for(int i=0;i<16;i+=2){
			int find=0;
			substring=decrypted_message.substr(i,len);
			while(1){
